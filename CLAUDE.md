...
| Does clearing the quota end the board? | Yes. Every control is withdrawn and a single button, centred, moves on to the next level or the next endless round. This reverses the earlier answer — play used to continue so routes could be extended for a bigger score — because a finished board with a full quota bar and a live tray reads as unfinished. One consequence is that a Pivot Token earned by the route that clears a campaign level cannot be spent, since campaign levels do not carry tokens; endless rounds do. |
