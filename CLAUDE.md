...
| Why do springs and hubs breathe? | A spring's ring grows from its centre to its rim and a hub's collapses inward, which is the "radiating versus converging silhouette" the art guide asks for in section 9. Until then the only thing separating a source from a destination was background colour, which a colour-blind player does not have. The motion is therefore an accessibility channel that happens to look alive, not decoration that happens to be accessible. |
